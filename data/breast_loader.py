...
class Breast_Dataset(Dataset):
    def __init__(self, image_dir, concept_file, labeled_ratio, training, seed=42, transform=None):
        ...
        self.data = pd.read_csv(concept_file)
        logging.info(f"Loaded {len(self.data)} samples from {concept_file}")

        if 'Image_filename' not in self.data.columns:
        ...
        self.concept_columns = self.data.columns[3:].tolist()
        logging.info(f"Total {len(self.concept_columns)} concepts: {self.concept_columns}")

        self.data['label'] = pd.Categorical(self.data['classification_malignant']).codes
        ...
        if training:
            ...
            for _, row in self.data.iterrows():
                class_count[row['classification_malignant']] += 1

            labeled_count = defaultdict(int)
            for idx, row in self.data.iterrows():
                class_label = row['classification_malignant']
